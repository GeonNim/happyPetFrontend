...
const MyPage = ({ onLogout }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [reservationsResponse, inquiriesResponse] = await Promise.all([
          axios.get(`${secrets.REACT_APP_MY_DOMAIN}/get_reserv/${userid}`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get(`${secrets.REACT_APP_MY_DOMAIN}/get_inq/${userid}`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
  ...
  const handleAccountDeletion = async () => {
    ...
    if (confirmDeletion) {
      ...
      try {
        await axios.delete(`${secrets.REACT_APP_MY_DOMAIN}/delete_account/`, {
          headers: {
