...
function PostList() {
  ...
  useEffect(() => {
    const getPostList = async () => {
      try {
        const resp = await axios.get(`${secrets.REACT_APP_MY_DOMAIN}/get_inq/${userid}`);
  ...
  const deleteInq = async (post) => {
    ...
    if (confirmDeletion) {
      try {
        await axios.delete(`${secrets.REACT_APP_MY_DOMAIN}/delete_inq/${post.inq_idx}`);
        alert('삭제되었습니다.');
  ...
  {PostList.map((post) => (
    <div key={post.inq_idx} className="flex flex-col my-3">
      <div className="bg-[#f1f3ea] flex flex-col lg:flex-row px-3 py-5 rounded-lg relative min-h-[150px]">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center mb-4 lg:mb-0 lg:w-[40%]"> {/* 너비를 40%로 설정 */}
